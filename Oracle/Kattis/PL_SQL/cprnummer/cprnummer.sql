DECLARE
    ...
    procedure processInput(p_ipString VARCHAR2, p_ipArr IN OUT v_arr)
    is
    begin
        p_ipArr := v_arr();
        for i in 1..length(p_ipString)
        loop
            if substr(p_ipString, i, 1) <> '-'
            then
                p_ipArr.EXTEND;
    ...
    procedure processAnswer(p_ipArr IN v_arr, p_ansArr IN OUT v_arr)
    is
    begin
        p_ansArr := v_arr();
        for i in 1..p_ipArr.COUNT
        loop
            p_ansArr.EXTEND;
