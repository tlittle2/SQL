...
as
    ...
    begin
        with schema_check as (
            ...
            select nvl(count(1), 1) into l_differences
            from (
    ...
    begin
        ...
        print_or_execute('INSERT INTO '
                       ...
                       || ' a '
                       || sql_builder_pkg.get_where_in(sql_query_where_in, false)
                       || '('
...
BEGIN
    assert_pkg.is_true(check_schemas(p_cdc_table, p_stage_table) and check_schemas(p_cdc_table, p_target_table), 'SCHEMAS BETWEEN PROCESSING TABLES ARE NOT THE SAME. PLEASE INVESTIGATE');
