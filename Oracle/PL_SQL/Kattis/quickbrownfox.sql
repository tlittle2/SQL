...
BEGIN
    --get set of all lower case letters
    ...
    for i in 1..length(ip) loop
        if lower(substr(ip, i, 1)) member of c_arr then
        	ip_arr.EXTEND;
	...
	for i in 1..c_arr.COUNT loop
        if c_arr(i) not member of ip_arr then
        missing:= missing || c_arr(i);
        end if;
	end loop;

	--return if it's a panagram or not
	if length(missing) is null then
