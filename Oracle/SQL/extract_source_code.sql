select name, line
, decode (type, 'PACKAGE', lower(concat(name,'.pks')), 'PACKAGE BODY', lower(concat(name,'.pkb')), lower(concat(name,'.sql'))) as file_name
, decode(line, 1, concat('create or replace ',text), text) AS TEXT
from user_source
