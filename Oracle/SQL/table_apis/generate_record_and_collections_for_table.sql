...
select distinct 1 as s_order, table_name, 'type ' || lower(table_name) || '_rec is record(' as stmnt, 0 as column_id
...
from user_tab_columns where table_name in ('SALARY_DATA', 'INFA_GLOBAL') 
union all
select distinct 4 as s_order,table_name, 'type ' || lower(table_name) || '_tab is table of ' ||lower(table_name) || '_rec;' as stmnt, 32767 * 2 as column_id
from user_tab_columns where table_name in ('SALARY_DATA', 'INFA_GLOBAL') 
